...
def check_click(event):
    ...
    if (mouse_x - x_sup)**2 + (mouse_y - y_sup)**2 <= r2_sup**2:
        return ['super']
    for i in range(n):
...
def new_superball():
    ...
    r1_sup = 20
    r2_sup = 24
    vx_sup = (-1)^(randint(1, 2))*randint(5, 13)
    vy_sup = (-1)^(randint(1, 2))*randint(5, 13)
...
while name_to_write:
    screen.fill(DARK_BLUE)
    clock.tick(FPS)
    print_comment("Enter your name:", 200, 300, 72)
    print_comment("Please, small letters and numbers only", 200, 350, 36)
    print_comment("Enter to save your result, close this window to exit without saving", 200, 480, 36)
    print_comment(name, 200, 400, 72)
